...
{
    kd_tree* min;
    //cout<<a1->data[0]<<" "<<a2->data[0]<<" "<<a3->data[0]<<endl;
    if(a1==NULL && a2==NULL)
        return a3;
    else if(a2==NULL)
...
kd_tree* delete_tree(kd_tree* root,int* a,int dim,int cut)
{
    ...
    kd_tree* temp=t;
    //cout<<"t is "<<t->data[0]<<" "<<t->data[1]<<" "<<cut<<" "<<t<<" "<<q<<endl;
    if(t!=NULL)
    {
         ...
         if(t->right!=NULL)
        {
            ...
            //cout<<"min "<<min->data[0]<<" "<<min->data[1]<<" "<<cut<<endl;
            for(int i=0;i<dim;i++)
            {
                t->data[i]=min->data[i];
                //int temp=min->data[i];
...
min in dim 0 is 1 10
nearest neighbour of 60 60 is 70 70
